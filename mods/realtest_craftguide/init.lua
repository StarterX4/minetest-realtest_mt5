...
local progressive_mode = M.settings:get_bool("realtest_craftguide_progressive_mode", true)
...
local S = M.get_translator("realtest_craftguide")
...
local group_stereotypes = {
	...
	sand         = "default:sand",
	dye	     = "dye:red",
	water_bucket = "buckets:bucket_water",
...
function realtest_craftguide.register_craft_type(name, def)
	local func = "realtest_craftguide.register_craft_guide(): "
...
function realtest_craftguide.register_craft(def)
	local func = "realtest_craftguide.register_craft(): "
...
function realtest_craftguide.add_recipe_filter(name, f)
	local func = "realtest_craftguide.add_recipe_filter(): "
...
function realtest_craftguide.remove_recipe_filter(name)
	recipe_filters[name] = nil
end

function realtest_craftguide.set_recipe_filter(name, f)
	local func = "realtest_craftguide.set_recipe_filter(): "
	assert(name, func .. "filter name missing")
	assert(f and type(f) == "function", func .. "filter function missing")
...
function realtest_craftguide.get_recipe_filters()
...
function realtest_craftguide.add_search_filter(name, f)
	local func = "realtest_craftguide.add_search_filter(): "
	assert(name, func .. "filter name missing")
	assert(f and type(f) == "function", func .. "filter function missing")
...
function realtest_craftguide.remove_search_filter(name)
	search_filters[name] = nil
end

function realtest_craftguide.get_search_filters()
	return search_filters
end

local formspec_elements = {}

function realtest_craftguide.add_formspec_element(name, def)
	local func = "realtest_craftguide.add_formspec_element(): "
...
function realtest_craftguide.remove_formspec_element(name)
	formspec_elements[name] = nil
end

function realtest_craftguide.get_formspec_elements()
...
local function get_recipe_fs(data, iY)
	...
	if custom_recipe or shapeless or recipe.type == "cooking" then
		...
		if recipe.type == "cooking" then
			icon = "realtest_craftguide_cooking.png"
...
local function make_formspec(name)
	...
	fs[#fs + 1] = fmt("label[%f,%f;%s]",
		...
		0.22,
		ESC(fmt("%s / %u", data.pagenum, data.pagemax)))
...
local show_fs = function(player, name)
	if sfinv_only then
		...
		show_formspec(name, "realtest_craftguide", make_formspec(name))
	end
end

realtest_craftguide.add_search_filter("groups", function(item, groups)
...
if sfinv_only then
	...
	M.register_on_player_receive_fields(function(player, formname, fields)
		if formname == "realtest_craftguide" then
			on_receive_fields(player, fields)
		elseif fields.__realtest_craftguide then
			realtest_craftguide.show(player:get_player_name())
		end
	end)
...
function realtest_craftguide.show(name)
