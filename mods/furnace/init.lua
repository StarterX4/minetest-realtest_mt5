				...
				for i = 1,5 do
					...
					if cookeds[i] and cookeds[i].item and meta:get_float("src_time"..i) >= cookeds[i].time then
						...
						if inv:room_for_item("dst"..i,cookeds[i].item) then
							...
							local srcstack = inv:get_stack("src"..i, 1)
							srcstack:take_item()
							inv:set_stack("src"..i, 1, srcstack)
						else
							minetest.log("verbose", "[furnace] Could not insert '"..cookeds[i].item:get_name().."'")
						end
...
minetest.register_lbm({
	label = "Reset furnace sound",
	name = "furnace:reset_sound",
	nodenames = { "furnace:self_active" },
